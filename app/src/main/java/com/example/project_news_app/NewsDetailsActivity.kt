...
class NewsDetailsActivity : AppCompatActivity() {
    ...
    private fun saveVisitHistory(news: NewsData) {
        ...
        val visitHistoryNewsList = gson.fromJson(newsJson, Array<NewsData>::class.java).toMutableList()

        // Remove the news if it already exists to avoid duplicates
        visitHistoryNewsList.removeAll { it.newsId == news.newsId }

        // Add the news to the history
        visitHistoryNewsList.add(news)

        // Save the updated list back to SharedPreferences
        editor.putString("visit_history_news", gson.toJson(visitHistoryNewsList))
        editor.apply()
    }
